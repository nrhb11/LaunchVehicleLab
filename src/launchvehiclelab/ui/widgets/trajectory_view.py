"""Apple HIG Pro Telemetry Dynamics Canvas with synchronized playhead."""
...
def _setup_apple_hig_axes(ax, title: str, xlabel: str, ylabel: str) -> None:
    ...
    ax.set_title(title, color=TEXT_PRIMARY, fontsize=10, fontweight="bold", pad=10, loc="left")
    ...
    # Hairline minimalistic border
    for spine in ax.spines.values():
        spine.set_color(BORDER_HAIRLINE)
        spine.set_linewidth(0.5)

    # Only horizontal guide lines (no vertical clutter)
    ax.grid(True, axis="y", linestyle=":", alpha=0.3, color="#27272a", linewidth=0.5)
...
class TrajectoryView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.tabs.addTab(self.canvas_alt, "📈 Altitude Arc")
    ...
    def update_trajectory(self, traj: TrajectoryResult) -> None:
        ...
        # -------------------------------------------------------------
        # 1. Altitude vs Downrange Arc
        # -------------------------------------------------------------
        fig1 = self.canvas_alt.figure
        fig1.clear()
        ax1 = fig1.add_subplot(111)
        _setup_apple_hig_axes(ax1, "ASCENT TRAJECTORY FLIGHT ARC", "Downrange Distance (km)", "Altitude (km)")
